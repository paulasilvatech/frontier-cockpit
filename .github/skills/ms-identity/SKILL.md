---
name: ms-identity
description: Apply the ms-identity Design System (Microsoft identity) to any visual or editorial output signed by Frontier Cockpit Team as Software Global Black Belt. Forks the your-org DS, keeping the same logo shape and tokens but using the Microsoft 4-color palette in the logo, with different author attribution, role string, and contact (single email, no socials). Use for Microsoft-facing material, internal decks, workshop guides, client proposals, formal documents, multi-page playbooks, runbooks. Also covers deck export to vector PDF and to native editable PPTX (both per locale, with trilingual speaker notes), and the no-build multi-page trilingual playbook pattern (EN, PT-BR, ES). Trigger on "MS identity", "identidade Microsoft", "Software Global Black Belt", "ms-identity", "deck Microsoft", "playbook multi-page", "playbook trilingue", "gerar PDF do deck", "deck to PDF", "deck to PPTX", "PPTX editavel", "speaker notes", or any Microsoft-authored output. Required to keep personal and corporate channels separated.
---

# ms-identity Skill (v1.8.0)

Microsoft identity fork of the your-org Design System for Frontier Cockpit Team, Software Global Black Belt. Same visual system shape, formal author attribution, single-channel contact, multi-page trilingual playbook architecture.
...
| `assets/deck.html` | The chrome bar still read `your-org · context platform stack` (personal brand label). Corrected to the identity-locked `Frontier Cockpit Team \| Software Global Black Belt`. |
...
## When to use this skill vs your-org-ds

| Use **ms-identity** | Use **your-org-ds** |
|---|---|
...
The `</>` SVG logo uses **Microsoft 4-color palette**, not your-org personal:
...
| Contact | `frontier-cockpit@example.com` (mailto link, single channel) |
...
- `@your-org` (personal GitHub)
- `your-handle`, LinkedIn (personal)
- `example.com` (personal site)
- `Microsoft Americas`, `Frontier Cockpit` (deprecated, use just `Software Global Black Belt`)
...
python3 scripts/deck_to_pdf.py \
  ...
  --title "Event Name" --author "Frontier Cockpit Team, Software Global Black Belt" \
...
- Sign as `AI-Native Software Engineer`, that is your-org identity, not MS.
...
### Bundled scripts
- `scripts/deck_to_pdf.py`, converts any ms-identity deck HTML to a vector PDF in a named locale (`--locale` required), with MS-identity validation built in (warns if `AI-Native Software Engineer` or `@your-org` leak in MS material).
- `scripts/build_pptx.py`, converts a ms-identity deck HTML to a native, fully editable PPTX in a named locale (`--locale` required), with native PowerPoint speaker notes. Reads the deck's embedded `deck-manifest` and `I18N` blocks. See the export sections of `references/deck.md`.
