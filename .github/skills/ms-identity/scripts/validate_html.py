#!/usr/bin/env python3
"""Validate an HTML deliverable against the ms-identity identity rules.
...
def validate(path, check_assets=False):
    ...
    if "frontier-cockpit@example.com" not in content:
        errors.append("Microsoft contact email (frontier-cockpit@example.com) not found")
...
def main(argv):
    ...
    if total:
        ...
        return 1
    print("\nAll HTML deliverables pass the ms-identity identity gate.")
    return 0
