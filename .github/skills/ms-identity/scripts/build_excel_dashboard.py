#!/usr/bin/env python3
"""Build a ms-identity Office Excel dashboard, non-destructively.

This is the bundled helper for the Excel module of the ms-identity skill
...
# ---- ms-identity Office tokens (ARGB) ----
FONT = 'Segoe UI'
MONO = 'Consolas'
INK = 'FF201F1E'
BLUE = 'FF0078D4'
...
def set_props(wb, title, desc, subject='GitHub Copilot Usage-Based Billing',
    ...
    p = wb.properties
    p.creator = 'Frontier Cockpit Team, Software Global Black Belt'
    p.lastModifiedBy = 'Frontier Cockpit Team, Software Global Black Belt'
    p.title = title
    p.subject = subject
    p.keywords = keywords
    p.description = desc
...
def _cli(src, dst):
    ...
    footer(ws, 12, 'Author: Frontier Cockpit Team, Software Global Black Belt, Microsoft.')
    for nm in existing:
        restyle(wb[nm])
    set_props(wb, 'ms-identity Excel dashboard', 'Smoke test output.')
    wb.save(dst)
