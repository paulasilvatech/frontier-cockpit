# Editorial Spine: Trilingual Single-Source Pattern (ms-identity)
...
The spine pattern is shared with `your-org-ds`. The difference for MS material is in the **meta** keys (role, contact, signature) and the **trilingual policy** (the EN tagline does not get translated, see below).
...
Same structure as your-org-ds spine, with these MS-identity overrides in `meta`:

```json
{
  "$version": "1.0.0",
  "$author": "Frontier Cockpit Team",                    // not "AI-Native Software Engineer"
  "$date": "2026-04-22",
  ...
  "meta": {
    "en": {
      ...
      "contact": "frontier-cockpit@example.com",   // mandatory single channel
      "audience": "...",
      "duration": "...",
      "output": "..."
    },
...
- **Personal socials** (`@your-org`, LinkedIn, example.com), never appear in MS spine.
...
- [ ] Contact is `frontier-cockpit@example.com` (single channel; no socials)
- [ ] No `AI-Native Software Engineer` anywhere in any locale of any spine file
- [ ] No `@your-org`, `your-handle`, or `example.com` in any spine file
