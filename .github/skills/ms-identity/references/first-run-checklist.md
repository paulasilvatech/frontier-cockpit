...
The single gate that makes a ms-identity deliverable correct on the first execution. Walk the common block for every artifact, then the format-specific block for what you are producing. If an item fails, fix it and re-check, do not deliver.
...
- [ ] Author is "Frontier Cockpit Team", role is "Software Global Black Belt" with no region suffix.
- [ ] `frontier-cockpit@example.com` is the only contact; no personal social handles.
