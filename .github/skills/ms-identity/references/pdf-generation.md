...
This is the canonical pattern for converting any ms-identity HTML deck (the engine bundled in `assets/deck.html`) into a vector PDF. The output is selectable text, not a screenshot. Every ms-identity deck deliverable should ship in BOTH HTML and PDF format.
...
For ms-identity identity, `/Author` should always be `Frontier Cockpit Team, Software Global Black Belt` (formal role, no comma alternatives like "AI-Native Software Engineer").
...
assert 'frontier-cockpit@example.com' in r.pages[0].extract_text() or \
       'frontier-cockpit@example.com' in r.pages[-1].extract_text()
# Sanity-check forbidden personal-identity strings
combined = '\n'.join(p.extract_text() for p in r.pages)
assert 'AI-Native Software Engineer' not in combined
assert '@your-org' not in combined
