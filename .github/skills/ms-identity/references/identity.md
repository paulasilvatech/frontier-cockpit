# ms-identity Identity Reference

Canonical strings, logo markup, and forbidden patterns for the Microsoft identity fork.

The ms-identity identity shares the *shape* of the your-org visual system (same logo geometry, same component structure) but is a distinct identity. The palette in the logo, the author role, and the contact channel are all different. "Same shape, different identity" is the whole point of the fork. Never describe the two as interchangeable.
...
Role (full):    Frontier Cockpit Team, Software Global Black Belt
Chrome-bar form: Frontier Cockpit Team | Software Global Black Belt   (mono uppercase, letter-spacing 0.16em)
Contact:        frontier-cockpit@example.com   (single channel)
Tagline (EN):   Building the future of software development with AI and Agentic DevOps
```

The role is **Software Global Black Belt**. Not "Frontier Cockpit", not "Microsoft Global Black Belt", not abbreviated. It stands alone, with no organization and no region.
...
The mark is the four-color `</>` shape. The geometry is shared with your-org, but **the fill colors are the Microsoft palette**:
...
```html
<svg viewBox="0 0 1914 1062" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="frontier-cockpit" style="width:22px;height:auto;flex-shrink:0;">
  <title>frontier-cockpit</title>
...
This string never translates and never gets a project-specific suffix. A chrome bar that reads `your-org · <project name>` is the personal identity leaking in; replace it with the canonical string above.

The exact markup differs slightly by format, and the format's asset is the source of truth:

- **Deck** (`assets/deck.html`): `<div class="deck-brand">` with `.deck-brand__squares` and `<span class="deck-brand__text">Frontier Cockpit Team | Software Global Black Belt</span>`. Engine CSS styles it; the `.deck-brand__text` uses `--ps-font-mono` and `--ps-color-ink-3`.
- **Playbook** (`assets/playbook/index.html`): `<header class="chrome">` with `.chrome__brand`, the `</>` SVG as `.chrome__logo`, and `<span class="chrome__name">Frontier Cockpit Team | Software Global Black Belt</span>`.
...
```html
<div class="meta-grid">
  <div class="meta-item"><span class="meta-label" data-i18n="labels.author">Author</span><span class="meta-value">Frontier Cockpit Team</span></div>
...
<section class="slide slide--dark" style="--accent: var(--ps-color-ms-blue-500);">
  ...
  <a href="mailto:frontier-cockpit@example.com" style="display:inline-flex; align-items:center; gap:12px; padding:14px 20px; border:1px solid rgba(240,240,240,.2); border-radius:8px; text-decoration:none; color:var(--ps-color-dark-ink);">
    ...
    <span style="font-size:15px;">frontier-cockpit@example.com</span>
  </a>
</section>
```

Whichever variant you use, the contact is `frontier-cockpit@example.com` and nothing else. No LinkedIn, no GitHub, no personal site.
...
| `@your-org` | Personal GitHub handle | (omit, no socials in MS) |
| `your-org ·` (as a chrome-bar label) | Personal brand label leaking into the chrome bar | `Frontier Cockpit Team \| Software Global Black Belt` |
| `your-handle` | Personal LinkedIn handle | (omit) |
| `example.com` | Personal site | (omit) |
| `linkedin.com/in/your-handle` | Personal LinkedIn URL | (omit) |
...
| `Frontier Cockpit` | Deprecated regional abbreviation | `Software Global Black Belt` |
...
Note that `your-org` as a bare word is allowed when it names the design system itself (for example, "the your-org Design System"). It is forbidden as an author label, a handle, or a chrome-bar brand string.
...
for s in "AI-Native Software Engineer" "@your-org" "your-handle" \
         "example" "Microsoft Americas" "Frontier Cockpit" \
...
count=$(grep -Ec "(deck-brand__text|chrome__name)[^>]*>[^<]*your-org" "$file" 2>/dev/null || echo 0)
echo "  personal brand label in chrome bar: $count"

echo ""
echo "Required strings (each must be >= 1):"
for s in "Software Global Black Belt" "frontier-cockpit@example.com"; do
  count=$(grep -Fc "$s" "$file" 2>/dev/null || echo 0)
  echo "  '$s': $count"
done
```
