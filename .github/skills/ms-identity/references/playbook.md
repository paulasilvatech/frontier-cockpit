...
applies-to: ms-identity
---

# ms-identity playbook reference
...
| Chrome bar text | `Frontier Cockpit Team | Software Global Black Belt` |
| Contact | `frontier-cockpit@example.com` |
| Tagline | `Building the future of software development with AI and Agentic DevOps` |

The chrome bar shows the `</>` SVG logo (4 paths) using the **Microsoft 4-color palette**, not the your-org personal palette:
...
**Forbidden in MS playbooks**: `AI-Native Software Engineer`, `@your-org`, GitHub or LinkedIn handles, `example.com`, `Microsoft Americas`, `Frontier Cockpit`. The personal palette `#FF3133 / #7ED956 / #FFDE59 / #39B8FF` must never appear in the logo SVG.
...
grep -rE '(AI-Native Software Engineer|@your-org|your-handle|example|Microsoft Americas|Frontier Cockpit)' .
...
for f in *.html; do
  ...
  grep -q 'frontier-cockpit@example.com' "$f" || echo "❌ $f missing email"
done
```
...
- Using the your-org personal palette `#FF3133 / #7ED956 / #FFDE59 / #39B8FF` in the logo SVG. The MS playbook always uses Microsoft `#F25022 / #7FBA00 / #FFB900 / #00A4EF`.
- Putting `Frontier Cockpit Team` or `Software Global Black Belt` inside a content module, then translating it. These strings stay in the HTML, hardcoded EN, never in JSON.
